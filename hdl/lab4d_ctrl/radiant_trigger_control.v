...
// We still want two runmodes, one with dual-bank switching and one with
// single-bank switching. That's what "trigger repeat" will handle now. Instead of
// being a 2-bit value it'll really just be a one-bit value.
// Tried other more elegant ways to handle this, just interacts way too weirdly with
// the rest of the firmware.
//
// In single-bank switching mode we do
// 0, 16, 1, 17, 2, 18, 3, 19 etc. and repeat from the beginning.
// Then when a trigger occurs, we wait until the post-trigger time and
// move to the next bank.
//
// In dual-bank switching mode we do
// 0, 16, 1, 17, 2, 18, 3, 19 etc. and then when we get to
// window[2:0] == 111, we jump to
// 4, 20, 5, 21, 6, 22, 7, 23 etc.
//
// We handle this by having an additional pseudo-window bit.
...
module radiant_trigger_control #(parameter NUM_WR=4, parameter WR_DELAY=0)(
		input clk_i,
		input sys_clk_i,
